...
import {
  type OrchestratorReplacementFailure,
  useUiStore,
...
function SettingsBody({
  ...
  const activeOrchestrator = newestActiveOrchestrator(
    workspace?.sessions ?? [],
  );
  ...
  const [form, setForm] = useState({
    ...
    workerModel:
      config.worker?.agentConfig?.model ?? config.agentConfig?.model ?? "",
    orchestratorModel:
      config.orchestrator?.agentConfig?.model ??
      config.agentConfig?.model ??
      "",
    workerMode:
      config.worker?.agentConfig?.mode ?? config.agentConfig?.mode ?? "",
    orchestratorMode:
      config.orchestrator?.agentConfig?.mode ?? config.agentConfig?.mode ?? "",
  ...
  const missingRequiredAgent =
    form.workerAgent === "" || form.orchestratorAgent === "";
  ...
  const effectiveIntakeRepo =
    form.intakeRepo.trim() || deriveGitHubRepo(project.repo);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (
        ...
        (activeOrchestrator &&
          activeOrchestrator.provider !== form.orchestratorAgent)
      ) {
        try {
          ...
          const replacementFailure: OrchestratorReplacementFailure = {
            message:
              error instanceof Error
                ? error.message
                : t("settings.project.replaceOrchestratorFailed"),
          ...
          <ProjectAgentsSettingsView
            ...
            workerModelArea={
              <AgentModelField
                ...
                onModelChange={(workerModel) =>
                  setForm((f) => ({ ...f, workerModel }))
                }
                onModeChange={(workerMode) =>
                  setForm((f) => ({ ...f, workerMode }))
                }
              />
            }
            orchestratorArea={
              <RequiredAgentField
                ...
                disabled={agentsQuery.isFetching && agentCatalog === undefined}
                invalid={
                  validationError !== null && form.orchestratorAgent === ""
                }
                onChange={(v) =>
                  setForm((f) => ({
                    ...f,
            ...
            orchestratorModelArea={
              <AgentModelField
                ...
                onModelChange={(orchestratorModel) =>
                  setForm((f) => ({ ...f, orchestratorModel }))
                }
                onModeChange={(orchestratorMode) =>
                  setForm((f) => ({ ...f, orchestratorMode }))
                }
              />
            }
            permissions={{
            ...
            <>
              <ProjectWorkflowSettingsView
                ...
                onBranchChange={(defaultBranch) =>
                  setForm((f) => ({ ...f, defaultBranch }))
                }
                onPrefixChange={(sessionPrefix) =>
                  setForm((f) => ({ ...f, sessionPrefix }))
                }
                labels={{
...
function AgentModelField({
  ...
  useEffect(() => {
    if (revalidationQuery.data) {
      queryClient.setQueryData(
        agentModelsQueryKey(agentId, projectId),
  ...
  if (isMode) {
    ...
    return (
      <>
        ...
        {warning && (
          <p className="px-1 text-xs leading-row text-warning">{warning}</p>
        )}
      </>
    );
  }
  ...
  const modelIsInCatalog =
    catalog?.models?.some((item) => item.id === model) ?? false;
              ...
              <input
                ...
                placeholder={
                  query.isFetching
                    ...
                    : t("settings.project.agentDefault")
                }
              />
              {hasCatalog && (
                <AgentModelCombobox
...
function ModelRefreshButton({
  ...
  return (
    <button
      type="button"
      aria-label={t("settings.models.refreshAria", {
        label: label.toLocaleLowerCase(),
      })}
      title={t("settings.models.refreshAria", {
        label: label.toLocaleLowerCase(),
      })}
    ...
    >
      <RefreshCw
        className={cn("size-icon-sm", pending && "animate-spin")}
        aria-hidden="true"
      />
    </button>
  );
