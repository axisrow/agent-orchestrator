...
type SessionRecord struct {
	...
	// SessionEnv is a durable JSON object of additive per-session overrides. It
	// is kept off the general session read model because values may be
	// credentials; the launch path merges it over project and role defaults.
	// A JSON string, rather than a map, keeps SessionRecord comparable for the
	// existing lifecycle CAS tests and value semantics.
	SessionEnv string          `json:"-"`
