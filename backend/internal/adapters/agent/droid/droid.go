...
// EmitsSubmitActivity signals Droid fires a user-prompt-submit hook under AO's
// launch. See ports.ActivitySignaler.
func (p *Plugin) EmitsSubmitActivity() bool { return true }

// EmitsBlockedActivity is false: droid reports its undiscriminated Notification
// as waiting_input — it installs no tool hooks, so a blocked state could never
// be cleared mid-turn. confirmActive must not nudge it (an Enter could answer
// a pending decision it cannot report as blocked). See ports.ActivitySignaler.
